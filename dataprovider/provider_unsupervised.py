...
def split_data_by_chronological_strategy(X, y, metadata):
    """基于时间策略分割数据：训练集(Monday+Tuesday正常) + 验证集(Wednesday正常) + 测试集(其余)"""

    window_metadata = metadata['window_metadata']

    # 定义每个集合的文件和条件
    train_files = ['Monday-WorkingHours', 'Tuesday-WorkingHours']
    val_files = ['Wednesday-workingHours']
    test_files = ['Wednesday-workingHours', 'Thursday-WorkingHours-Morning-WebAttacks',
                  'Thursday-WorkingHours-Afternoon-Infilteration', 'Friday-WorkingHours-Morning',
                  'Friday-WorkingHours-Afternoon-PortScan', 'Friday-WorkingHours-Afternoon-DDos']
    ...
    for i, window_meta in enumerate(window_metadata):
        source_file = window_meta.get('source_file', '')
        is_malicious = window_meta.get('is_malicious', 0)

        if source_file in train_files and is_malicious == 0:
            # 训练集：Monday + Tuesday的正常窗口
            train_indices.append(i)
        elif source_file in val_files and is_malicious == 0:
            # 验证集：Wednesday的正常窗口
            val_indices.append(i)
        elif source_file in test_files:
            # 测试集：Wednesday异常窗口 + Thursday全天 + Friday全天
            if source_file == 'Wednesday-workingHours' and is_malicious == 1:
                test_indices.append(i)
            elif source_file != 'Wednesday-workingHours':
                test_indices.append(i)
    ...
    total_samples = len(X)
    print(f"基于时间策略的数据分割:")
    print(f"  训练集: {len(X_train)} ({len(X_train) / total_samples * 100:.1f}%) - 纯正常流量")
    print(f"  验证集: {len(X_val)} ({len(X_val) / total_samples * 100:.1f}%) - 纯正常流量")
    print(f"  测试集: {len(X_test)} ({len(X_test) / total_samples * 100:.1f}%) - 混合流量")

    # 验证训练集和验证集的纯净度
    train_anomaly_rate = np.mean([window_metadata[i]['is_malicious'] for i in train_indices]) * 100
    val_anomaly_rate = np.mean([window_metadata[i]['is_malicious'] for i in val_indices]) * 100
    test_anomaly_rate = np.mean([window_metadata[i]['is_malicious'] for i in test_indices]) * 100

    print(f"  训练集异常率: {train_anomaly_rate:.2f}%")
    print(f"  验证集异常率: {val_anomaly_rate:.2f}%")
    print(f"  测试集异常率: {test_anomaly_rate:.2f}%")
