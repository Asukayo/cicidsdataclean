...
class DynamicalContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, scores, attn_mask, norm_matrix):
        ...
        # 在 forward 方法中，替换第66-67行
        epsilon = 1e-8  # 防止除零和log(0)
        pos_sum = pos_scores.sum(dim=-1) + epsilon
        all_sum = all_scores.sum(dim=-1) + epsilon
        clustering_loss = -torch.log(pos_sum / all_sum)
