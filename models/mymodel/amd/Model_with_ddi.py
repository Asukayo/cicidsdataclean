...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 添加通道注意力机制
        self.crosschannelTransformer = Trans_C(dim = 128,depth=4,heads=10,mlp_dim=256,dim_head=64,dropout=0.7,seq_len=100,d_model=128)

        self.trend_linear_before_classifier = nn.Linear(100, self.feature_dim)

        self.seasonal_linear_before_classifier = nn.Linear(128, self.feature_dim)
    ...
    def forward(self, x):
        ...
        trend_init, seasonal_init, _ = self.stl_decomp(x_normalized) # stl 输出的形状为[batch_size,channels,seq_len]
        ...
        seasonal_output = self.seasonal_linear_before_classifier(cross_output)
