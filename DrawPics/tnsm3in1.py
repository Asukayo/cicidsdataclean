...
def beauty_plot(ax, x_label, y_label):
    ...
    # solid light-grey grid (EPS has no transparency)
    ax.grid(True, color='0.8', linestyle='--', linewidth=1)
    ax.set_axisbelow(True)


color_main, color_sub = '#1F77B4', '#D62728'
...
ax1.plot(window_sizes, f1_scores_w, marker='o', linewidth=3, markersize=8,
         color=color_main, label='F1-Score')
ax1.scatter(window_sizes[1], f1_scores_w[1], s=250, color='#FFD700', marker='*',
            edgecolors='black', zorder=5, label='Selected')
ax1_right = ax1.twinx()
ax1_right.plot(window_sizes, annotation_counts, marker='s', linewidth=3, markersize=8,
               color=color_sub, linestyle='--', label='Annotations')
...
ax2.scatter(alpha_values[2], f1_scores_a[2], s=250, color='#FFD700', marker='*',
...
ax3.scatter(patch_sizes[1], f1_scores_p[1], s=250, color='#FFD700', marker='*',
            edgecolors='black', zorder=5)
ax3_right = ax3.twinx()
ax3_right.plot(patch_sizes, num_patches, marker='s', linewidth=3, markersize=8,
               color=color_patch, linestyle='--')
...
fig.savefig('Fig5.eps', format='eps', bbox_inches='tight', facecolor='white')
fig.savefig('Fig5.tiff', dpi=600, bbox_inches='tight', facecolor='white',
            pil_kwargs={'compression': 'tiff_lzw'})
plt.close(fig)
print("Fig5 saved (EPS + TIFF).")