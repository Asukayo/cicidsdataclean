"""
无监督异常检测 · FullModel 训练脚本
====================================
基于 autoWeightsScript.py 修改，专为 FullModel 设计。
FullModel = Trend-Stable 分解 + Cosine 原型记忆库 + 频域自适应加权

新增保存内容（供论文画图）：
  - freq_log_var:   频率不确定性参数 → 绘制 exp(-s_f) 曲线
  - prototypes:     学到的原型向量 → t-SNE 可视化
  - trend_filter:   低通卷积核权重 → 绘制各通道的滤波器频响

用法：
  python FullModelScript.py --data_dir /path/to/data --model FullModel
"""
...
def build_model(args, device):
    ...
    elif args.model == 'TCNAE_AutoFreq':
        model = cls(
            input_dim=args.n_features,
            window_size=args.window_size,
            kernel_size=args.tcn_kernel_size,
...
def main(args):
    ...
    if args.save_dir:
        ...
        if hasattr(model, 'freq_log_var'):
            ...
            print(f"\n  [Saved] freq_log_var → {args.model}_learned_freq_log_var.npy")
            print(f"          绘制 exp(-log_var) 曲线，验证低频自动降权")

        # 导出原型向量（供 t-SNE 可视化 / 原型分析）
        if hasattr(model, 'encoder') and hasattr(model.encoder, 'memory'):
            prototypes = model.encoder.memory.prototypes.detach().cpu().numpy()
            np.save(os.path.join(args.save_dir, f'{args.model}_prototypes.npy'), prototypes)
            print(f"  [Saved] prototypes ({prototypes.shape}) → {args.model}_prototypes.npy")

        # 导出低通卷积核权重（供滤波器频响分析）
        if hasattr(model, 'trend_extractor'):
            filter_weights = torch.softmax(model.trend_extractor.raw_weight, dim=-1).detach().cpu().numpy()
            np.save(os.path.join(args.save_dir, f'{args.model}_trend_filter.npy'), filter_weights)
            print(f"  [Saved] trend_filter ({filter_weights.shape}) → {args.model}_trend_filter.npy")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='无监督异常检测 · FullModel 训练脚本')
    ...
    g.add_argument('--model', type=str, default='FullModel',
    ...
    g = parser.add_argument_group('Trend Decomposition')
    g.add_argument('--trend_kernel_size', type=int, default=25,
                   help='低通卷积核大小；约 1/4 窗口长度')

    g = parser.add_argument_group('LowPass')
    g.add_argument('--smooth_weight', type=float, default=0.3)

    g = parser.add_argument_group('Memory Prototype Bank')
    g.add_argument('--num_prototypes', type=int, default=16,
                   help='原型数量 K')
    g.add_argument('--proto_tau', type=float, default=0.1,
                   help='cosine attention 温度参数')
    g.add_argument('--sparse_weight', type=float, default=0.01,
                   help='原型稀疏正则权重 λ2')
    g.add_argument('--split_after', type=int, default=2,
                   help='在第几个 TCN block 之后插入原型库')

    g = parser.add_argument_group('Frequency Branch')
